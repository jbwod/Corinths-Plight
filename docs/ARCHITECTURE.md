...
| D1 migrations | `migrations/0001_platform_and_rules.sql` through `0022_game_master_skirmish_policy.sql` | Twenty-two additive schema artifacts in the repository; production is recorded only through `0007`, so later gameplay, operations, Game Master, authoring, custom-runtime, and skirmish-policy migrations are local until a deployment is actually recorded |
...
| Game Master and map authoring | `worker/routes/game-master.ts`, `worker/game-master-authoring.ts`, `worker/game-master-runtime.ts`, `src/components/GameMasterConsole.tsx`, migrations `0019`–`0022` | Admin-only local vertical slice: live campaign controls and `game-master-recovery@1` exceptional correction are grant-scoped, receipt-idempotent and audited; five deterministic map presets save/edit/publish immutable revisions and create exact-pinned recruiting campaigns that use the ordinary join/deployment/runtime path. New custom content pins `game-master-skirmish@1` and `public-v1-economy@1`; legacy custom `@1` remains fail-closed. Standalone-custom strategic placement remains blocked. |
...
| Migrations and idempotent seed artifacts | Local head `0022`; production head `0007` | Fresh empty D1 replay through all twenty-two migrations and all nine seeds twice passes integrity/FK checks across 130 application tables; no Game Master migration or runtime is claimed as deployed until a new production deployment is recorded |
...
| Helion/Corinth strategic schema and fixture | Complete as development data only | Fresh 0001–0022 replay and all nine seeds twice passed; production seeds still create no strategic world or campaigns |

...
The Campaign DO's tactical order upsert and cancellation require an actor-scoped command ID plus expected campaign/order revisions; clock update requires a command ID plus expected campaign version. Global-grant-scoped Game Master clock/objective/spawn/pause/resume/resolve commands use the same actor-scoped request-hash and campaign-version discipline and project private D1 receipts/audits. Matching retries replay; changed-payload command reuse and stale revisions fail. The DO mutation and D1 Game Master audit projection remain separate stores, so the command lifecycle still needs reconciliation evidence before a general replay guarantee.

...
- Custom-map application profile: `corinth-custom-map-terrain-application@2`; owner-approved application interpretation, not canonical V5
- Custom-skirmish application profile: `game-master-skirmish@1`; owner-approved application interpretation for exact Game Master scenario content `@2`, not canonical V5
- Game Master recovery profile: `game-master-recovery@1`; owner-approved exceptional global-administrator correction, not canonical V5 repair or ordinary player recovery
...
| Attack action | `canonical_active` plus V5 rulings | `foundation_partial`: one server-owned Attack activation per unit fires every eligible fitted weapon once and implements FS cap, Armor/AP/Defense, governed rear effects, range, direct LOS, indirect spotting, ammo, cooldown, Rush damage, simultaneous commit, the Fighter's forward travel-path arc, and Bomber fly-over geometry. Infantry Light AT is a Range-1 disposable modifier: the player may spend one to three fitted charges, each adding +1 AP to the single Infantry rifle attack without adding another damage die. Melee and wider aerospace attack rules remain deferred. |
...
| Hex geometry, pathing, LOS, capacity | Scenario-owned implementation of canonical measurement/LOS | `foundation_partial`: axial distance, adjacency, battlefield bounds, deterministic cost-aware pathfinding, quarter-distance movement cost, hostile formation blocking, LOS blockers, sensors, and per-hex capacity exist. Custom maps pin `corinth-custom-map-terrain-application@2`, including all current biome/point/edge profiles; older authored road/elevation/river defaults are not thereby fully reconciled. |
...
| Medic, engineer, artillery, cargo, logistics and Small Supply actions | `canonical_active`/`active_provisional` | `foundation_partial`: governed Load/Unload, Logi/Light Vehicle/IFV cargo, Artillery towing, narrow support resupply, Medic First Aid, Engineer Repair/construction, Crew Repair, Artillery state/Bombardment and Infantry Trench Upgrade have resolver/UI/persistence paths. Eligible Infantry enter authored buildings for `0.25` Speed, persist `GARRISONED` occupancy, and receive non-stacking `+1 Armor` cover against outside fire. A Field Bridge spends the published two Small Supply and marks an exact river edge without deleting the river; its combat/repair lifecycle remains blocked. HAT clear-route Infantry/Light Vehicle airdrop is active with hazardous destinations rejected. Scan and Deploy Drone remain rejected; hazardous HAT results, MASH and Funnel remain deferred. |
| Thirteen non-orbital V5 starting classes | `canonical_active` | `foundation_partial`: D1 catalogues all thirteen plus three companion-only classes, while generated tactical execution now resolves a playable subset for every canonical class. Companion-only classes without a registered handler fail closed before campaign execution. |
| Aerospace tactical rules | `canonical_active` | `foundation_partial`: generic VTOL flight/cargo, Fighter sortie/arc/ammunition/Interceptor, Bomber fly-over/ordnance, HAT clear-route airdrop, friendly-facility landing/takeoff, and landed Fighter/Bomber rearm resolve. Altitude transition and hazardous-drop outcomes remain deferred. |
| Optional Store equipment/refits | `catalogued` unless separately migrated | All 105 Store rows are represented in the generated catalogue with exact source effect text, cost status, slot and normalized unit restrictions. Flak Vests conditionally set unarmoured Infantry to Armor 1. Lightweight Anti-armour persists three charges and lets an Infantry Squad spend one to three at Range 1 for +1 AP each on its single rifle attack; submission, resolution, persistence, replay and reports expose the expenditure. Vehicle Optics and Drone Operator are release-blocked because their current handlers do not implement the advertised state/visibility effects and depend on unresolved interpretation; Orbital Drop Training records an eligibility mutation while orbital deployment remains disabled. Every unimplemented item is visible but blocked/catalogue-only rather than silently executable. |
...
| Game Master custom-scenario closure | `active application policy` | `foundation_executable` only for exact custom scenario content `@2`: `game-master-skirmish@1` defeats a force with no surviving Allied deployments, wins after at least one spawned enemy has existed and all enemies are lost, and otherwise defeats at round 12. Allied loss is checked first; enemy elimination precedes the round limit. Results use `public-v1-economy@1`. Legacy custom `@1` remains unupgraded and fails closed. |
| Game Master exceptional recovery | `active application policy` | `foundation_executable` only through the global-Game-Master route: `game-master-recovery@1` requires a campaign paused from planning and a destroyed deployment at a still-valid, occupiable, terrain-legal hex. The server restores active/on-map status, maximum governed health/ammunition, operational subsystems and clears transient damage/status/cooldown state while preserving explicitly permanent effects. Persistent reconciliation is Allied-only, audited, receipt-idempotent, and eventually projected across DO/D1. Ordinary gameplay cannot invoke it. |
...
The new `STEALTHED`, `PACKED`, `DEPLOYED`, `DUG_IN`, `EVASIVE`, `AIRBORNE`, `LANDED`, and `REARM_REQUIRED` definitions are server-owned statuses. Their presence in D1 does not authorize a client to set them or claim their effects. First Aid, Medic field resupply, Engineer vehicle Repair, IFV/MBT Crew Repair, Sandbag/Razor Wire/Tank Trap and bounded river-edge Field Bridge construction, Infantry Trench Upgrade, Artillery Deploy/Pack Up/Bombardment, aerospace Land/Take Off and landed aerospace Rearm are activated support actions. Bridge combat/repair lifecycle, MASH, Funnel, general Supply transfer, hazardous airdrop outcomes, wider Interceptor behavior, and sabotage remain disabled until their implementation overlay, server validation, deterministic resolver hook, fog projection, and tests agree.
...
- **Custom-map terrain application**: the owner-approved `corinth-custom-map-terrain-application@2` publishes explicit movement/LOS/cover/capacity for every `admin-map-vocabulary@2` biome and feature. Deep water and major mountains block ordinary ground movement; shallow/fresh water uses explicit amphibious tags; airborne movement crosses terrain blockers; rivers add `1`, exact bridges waive it, roads/paths multiply terrain cost by `0.5`/`0.75`, walls block ground movement and LOS, and all point features materialize governed structures/tags. This is application policy, not canonical V5.
- **Game Master skirmish closure**: the owner-approved `game-master-skirmish@1` terminal profile for exact custom content `@2`. It requires at least one enemy to have existed before enemy elimination can win, checks total Allied loss before enemy loss, and applies a round-12 defeat only while an enemy remains. This is application policy, not canonical V5.
- **Game Master exceptional recovery**: the owner-approved `game-master-recovery@1` global-administrator correction for a destroyed deployment while paused from planning. It restores only the server-defined state and is audited/idempotent; it is neither canonical V5 nor an ordinary repair, resurrection, or campaign-result mechanic.
...
|---|---|---|
| Infantry Squad | FS 6; Speed 1; Range 1; D6 | Dig In; entering an authored building costs `0.25` Speed and derives `GARRISONED` occupancy with `+1 Armor` against attacks from outside; paradrop into clear open space; cannot standard-attack low-orbit orbitals (`RC-V5-013`). |
...
The Infantry Squad through Heavy Air Transport are the thirteen canonical selectable classes targeted by this profile. The generated tactical catalogue executes a bounded playable subset for all thirteen. D1 can persist/catalogue more classes, which is not proof that they are playable. Power armor, irregulars, special forces, sappers, tank variants, artillery variants, medium/heavy mechs, and orbital hulls are not canonical selectable classes.
...
| Building garrison | `0.25` Speed on entry | **Active:** eligible Infantry Personnel entering an authored building pay the fixed V5 building-entry cost, including room-to-room movement. Ending inside derives `GARRISONED`; leaving removes it. The building grants the selected non-stacking `+1 Armor` cover against attacks from outside (`RC-COVER-001`). Garrison is occupancy derived from movement, not the rejected legacy `+2 Armor` action. |
...
| Bridge | 2 Small Supply | **Active bounded construction/crossing:** an Engineer places an exact adjacent river-edge Field Bridge; the river remains authored and that edge waives the crossing surcharge. Bridge attack, damage, repair, and destruction lifecycle remain unavailable despite the source-complete damage table. |
...
The generic VTOL's on-map tactical subset is active: each adjacent flight hex costs one Speed regardless of ground terrain, roads, elevation, rivers or fieldworks; it passes hostile ground formations, cannot spot ground targets, and uses the governed mutually-exclusive cargo profile. The Fighter sortie subset uses the same terrain-independent/hostile-passage flight path, cannot spot ground targets, may declare Evasive, carries one main-weapon shot, and can fire only within the forward 180-degree arc derived from the last travel step (or stored facing when stationary). When it declares a legal attack against an aerospace unit, `RC-V5-028` restricts that target's attack to a legal intercepting Fighter; a player must have selected one, while NPC ties use stable unit ID, and no legal interceptor means the attack is lost. The Bomber carries one D6 ordnance shot and must cross its target on the submitted flight path. The HAT uses its five-slot cargo table and may release manifested Infantry or Light Vehicles onto a clear hex of a straight submitted route for no Speed cost. VTOL, Fighter, Bomber and HAT may land at an authored compatible friendly facility and take off in a later order; takeoff resolves before movement and landing resolves after movement, and both cannot occur in one round. A landed Fighter or Bomber may spend a Primary Action at a friendly rearm facility to restore each authored ammunition capacity under `RC-V5-023`. Hazardous HAT outcomes remain deferred.
...
The Campaign DO's tactical order upsert and cancellation require an actor-scoped command ID plus expected campaign/order revisions; clock update requires a command ID plus expected campaign version. Global-grant-scoped Game Master clock/objective/spawn/pause/resume/resolve/recovery commands use the same actor-scoped request-hash and campaign-version discipline and project private D1 receipts/audits. Matching retries replay; changed-payload command reuse and stale revisions fail. Exceptional persistent recovery additionally validates and reconciles the exact server response into D1 Player Unit, deployment, mount, subsystem, status-effect and unit-history state. The DO mutation and D1 Game Master projection remain separate stores, so the command lifecycle still needs reconciliation evidence before a general replay guarantee.
...
- Special orders/actions: resolver hooks and tests for Melee Charge/Brawl, Stealth, Bridge combat/repair lifecycle, Funnel, wider Resupply, Assault, and Break Out/profile rejection behavior. Evasive, Dig In, movement-derived building Garrison, First Aid, Engineer Repair, Engineer-assisted Artillery Dig In, Sandbag/Trench/Razor Wire/Tank Trap fieldworks, bounded river-edge Field Bridge construction, Artillery Deploy/Pack Up/Bombardment, and Logi-to-Artillery Small Supply transfer are active; Load/Unload and finite-weapon Reload are narrow migrated UI/runtime paths; Scan and Deploy Drone still require authoritative effects/projection before activation.
...
- Canonical roster: all thirteen classes have executable generated subsets; remaining work completes class-specific mechanics rather than filling a missing class definition.
...
At the current local reconciliation point, the full root suite passes **106 Vitest files / 849 tests** (the committed Phase-0 baseline was 32/201). Coverage includes deterministic tactical and strategic helpers, quarter-distance simultaneous movement, generated terrain/structure cover and fieldwork movement penalties, multiweapon activations, Evasive movement/combat modifiers, VTOL/Fighter/Bomber flight, Fighter Interceptor restriction, authored-facility landing/takeoff/rearm, HAT clear-route drops, published enemy doctrine, custom-map publication/runtime and chunked state storage, bounded custom-skirmish closure, exceptional Game Master recovery/reconciliation, redaction, routes/LOS/capacity, combat/ammo/cooldowns, force/auth APIs, campaign contracts, strategic adapters/coordinator boundaries, governed cargo/towing/resupply, deployment validation, and support-action paths. The current Playwright gameplay baseline includes a four-round K-17 victory, the HAT manifested-drop composer, generated cargo/support controls, aerospace authority, Game Master map publication/bootstrap, and 390px overflow coverage.

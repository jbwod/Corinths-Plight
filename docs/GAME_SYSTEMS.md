...
| Attack action | `canonical_active` plus V5 rulings | `foundation_partial`: one server-owned Attack activation per unit fires every eligible fitted weapon once and implements FS cap, Armor/AP/Defense, governed rear effects, range, direct LOS, indirect spotting, ammo, cooldown, Rush damage, simultaneous commit, the Fighter's forward travel-path arc, and Bomber fly-over geometry. Melee and wider aerospace attack rules remain deferred. |

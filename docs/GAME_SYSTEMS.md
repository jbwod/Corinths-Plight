...
| Medic, engineer, artillery, cargo, logistics and Small Supply actions | `canonical_active`/`active_provisional` | `foundation_partial`: paired Load/Unload and finite-weapon Reload have narrow resolver/UI paths, with known action-ledger/occupancy/cargo gaps. Scan and Deploy Drone are rejected because their advertised visibility/state effects are not implemented. Heal, Repair, Construct, Bombardment/Funnel, general Resupply, Deploy/Pack Up, and Garrison remain deferred. |

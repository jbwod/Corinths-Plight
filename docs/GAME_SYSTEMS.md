...
| Medic, engineer, artillery, cargo, logistics and Small Supply actions | `canonical_active`/`active_provisional` | `foundation_partial`: paired Load/Unload, finite-weapon Reload, Combat Medic First Aid and field resupply, Engineer vehicle Repair, Sandbag Line construction and Infantry Trench Upgrade, and Artillery Deploy/Pack Up/Bombardment have resolver/UI/persistence paths. Sandbags cost one Small Supply; the no-Supply Primary upgrade creates a Trench that preserves Dig In along connected Trench hexes. Both grant Infantry +1 Armor against attacks from outside their hex. Scan and Deploy Drone are rejected because their advertised visibility/state effects are not implemented. MASH, remaining structures, Funnel, general Resupply, and Garrison remain deferred. |
...
Most rules below remain selected profile truth but deferred. `transferSupply`, `advanceBuildProgress`, and `canEquip` are deterministic helpers; the separate equipment/deployment slice adds narrow paired-cargo and finite-weapon Reload paths. First Aid, Engineer vehicle Repair, Sandbag Line construction and Trench Upgrade are active end-to-end support actions. Scan and Deploy Drone are rejected because their advertised state/visibility effects are incomplete. The slice does not activate MASH, general Resupply, the other structure types, or legacy Build Points.
...
- Special orders/actions: resolver hooks and tests for Melee Charge/Brawl, Stealth, the remaining construction types, Funnel, general Resupply, Garrison, Assault, and Break Out/profile rejection behavior. Evasive, Dig In, First Aid, Engineer Repair, Sandbag/Trench fieldworks, and Artillery Deploy/Pack Up/Bombardment are active; Load/Unload and finite-weapon Reload are narrow migrated UI/runtime paths; Scan and Deploy Drone still require authoritative effects/projection before activation.

...
| Aerospace tactical rules | `canonical_active` | `foundation_partial`: generic VTOL flight/cargo, Fighter sortie/arc/ammunition, Bomber fly-over/ordnance, HAT clear-route airdrop, friendly-facility landing/takeoff, and landed Fighter/Bomber rearm resolve. Altitude transition, Interceptor and hazardous-drop outcomes remain deferred. |
...
The generic VTOL's on-map tactical subset is active: each adjacent flight hex costs one Speed regardless of ground terrain, roads, elevation, rivers or fieldworks; it passes hostile ground formations, cannot spot ground targets, and uses the governed mutually-exclusive cargo profile. The Fighter sortie subset uses the same terrain-independent/hostile-passage flight path, cannot spot ground targets, may declare Evasive, carries one main-weapon shot, and can fire only within the forward 180-degree arc derived from the last travel step (or stored facing when stationary). The Bomber carries one D6 ordnance shot and must cross its target on the submitted flight path. The HAT uses its five-slot cargo table and may release manifested Infantry or Light Vehicles onto a clear hex of a straight submitted route for no Speed cost. VTOL, Fighter, Bomber and HAT may land at an authored compatible friendly facility and take off in a later order; takeoff resolves before movement and landing resolves after movement, and both cannot occur in one round. A landed Fighter or Bomber may spend a Primary Action at a friendly rearm facility to restore each authored ammunition capacity under `RC-V5-023`. Interceptor and hazardous HAT outcomes remain deferred.

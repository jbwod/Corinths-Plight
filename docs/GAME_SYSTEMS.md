...
The Campaign DO's tactical order upsert and cancellation require an actor-scoped command ID plus expected campaign/order revisions; clock update requires a command ID plus expected campaign version. All three commit SHA-256 request-hash/response receipts atomically with state and their Allied order event where applicable. Matching retries replay; changed-payload command reuse and stale revisions fail. Orders are current-round-only; cancellation increments the current order revision, and a later replacement updates the same unit/round aggregate. Operator pause/resume/resolve commands still lack the same idempotency/compare-and-set contract, so the command lifecycle remains partial rather than a general replay guarantee.
...
At the current local reconciliation point, the full root suite passes **59 test files / 476 tests** (the committed Phase-0 baseline was 32/201). Coverage includes deterministic tactical and strategic helpers, quarter-distance simultaneous movement, generated terrain/structure cover and fieldwork movement penalties, multiweapon activations, Evasive movement/combat modifiers, VTOL/Fighter/Bomber flight, Fighter Interceptor restriction, authored-facility landing/takeoff/rearm, HAT clear-route drops, published enemy doctrine, redaction, routes/LOS/capacity, combat/ammo/cooldowns, force/auth APIs, campaign contracts, strategic adapters/coordinator boundaries, governed cargo/towing/resupply, deployment validation, and support-action paths. The current Playwright gameplay baseline passes 8/8 locally, including a four-round K-17 victory, the HAT manifested-drop composer, generated cargo/support controls, aerospace authority and 390px overflow coverage.

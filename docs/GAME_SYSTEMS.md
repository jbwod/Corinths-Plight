...
| Medic, engineer, artillery, cargo, logistics and Small Supply actions | `canonical_active`/`active_provisional` | `foundation_partial`: paired Load/Unload, finite-weapon Reload, Combat Medic First Aid and field resupply, Engineer vehicle Repair and Sandbag Line construction, and Artillery Deploy/Pack Up/Bombardment have resolver/UI/persistence paths, with known action-ledger/occupancy/cargo gaps. Sandbags cost one Small Supply and grant Infantry +1 Armor against attacks from outside their hex. Bombardment consumes one Small Supply, requires a deployed battery plus friendly spotter, applies capped radius-one Defense suppression, stacks, and recovers by one in an unbombarded round. Scan and Deploy Drone are rejected because their advertised visibility/state effects are not implemented. MASH, remaining structures, Funnel, general Resupply, and Garrison remain deferred. |
...
The new `STEALTHED`, `PACKED`, `DEPLOYED`, `DUG_IN`, `EVASIVE`, `AIRBORNE`, `LANDED`, and `REARM_REQUIRED` definitions are server-owned statuses. Their presence in D1 does not authorize a client to set them or claim their effects. First Aid, Medic field resupply, Engineer vehicle Repair, Sandbag Line construction, and Artillery Deploy/Pack Up/Bombardment are activated support actions. MASH, remaining construction types, Funnel, general Supply transfer, crew repair, aerospace landing/rearm, airdrop, and sabotage remain disabled until their implementation overlay, server validation, deterministic resolver hook, fog projection, and tests agree.
...
Most rules below remain selected profile truth but deferred. `transferSupply`, `advanceBuildProgress`, and `canEquip` are deterministic helpers; the separate equipment/deployment slice adds narrow paired-cargo and finite-weapon Reload paths. First Aid, Engineer vehicle Repair, and immediate Sandbag Line construction are active end-to-end support actions. Scan and Deploy Drone are rejected because their advertised state/visibility effects are incomplete. The slice does not activate MASH, general Resupply, the other structure types, or legacy Build Points.
...
- Special orders/actions: resolver hooks and tests for Melee Charge/Brawl, Stealth, the remaining construction types, Funnel, general Resupply, Garrison, Assault, and Break Out/profile rejection behavior. Evasive, Dig In, First Aid, Engineer Repair, Sandbag Line construction, and Artillery Deploy/Pack Up/Bombardment are active; Load/Unload and finite-weapon Reload are narrow migrated UI/runtime paths; Scan and Deploy Drone still require authoritative effects/projection before activation.
...
At the current local reconciliation point, the full root suite passes **59 test files / 434 tests** (the committed Phase-0 baseline was 32/201). Coverage includes deterministic tactical and strategic helpers, quarter-distance simultaneous movement, generated terrain/structure cover, multiweapon activations, Evasive movement/combat modifiers, published enemy-doctrine materialization and permutation-stable target spreading, redaction, routes/LOS/capacity, combat/ammo/cooldowns, force/auth APIs, campaign contracts, strategic adapters/coordinator boundaries, effective-unit construction, deployment/cargo validation, and migrated equipment/support-action paths. The separate Playwright gameplay baseline passes 7/7 locally, covering the authentication shell, live strategic/tactical services, strategic Disembark resolution, tactical trust-boundary rejection, executable-action composer coverage, planning layers, and 390px overflow.

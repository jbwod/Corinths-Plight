...
The tactical expansion contains 43 transparent 1536×256 sprite sheets in `src/assets/tactical-sprites/generated/`. The explicit runtime allowlist selects exactly 36 sheets—29 player-unit sheets plus all seven Bug Swarm enemy definitions—and excludes seven superseded mech sheets from the client graph. Every sheet follows the six-frame order idle, move A, move B, attack, support and damaged, but that packing contract is not a camera-angle certification. The active `unit-light-mech-v3`, `unit-medium-mech-v4`, and `unit-heavy-mech-v3` revisions were generated from a reference-free plan-view brief and pass strict roof-only/nadir visual review. The other 33 active sheets remain `PROVISIONAL`, including infantry/medic/engineer families and the new enemy family that still need individual final visual approval.

The seven Bug Swarm sheets were generated on 2026-08-14 with OpenAI's built-in image generation tool and no third-party reference images. Their shared art direction uses strict nadir silhouettes, burnt-coral and bone chitin, dark-navy technical ink, and restrained cyan bioluminescence to distinguish hostile organisms from player hardware. Untouched RGBA source strips, packed sheets, exact SHA-256 hashes, catalogue status, subject briefs, and QA notes are recorded in `src/assets/tactical-sprites/swarm-sprite-provenance.json`. Drone, Warrior, and Heavy are experimental executable MVP definitions. Artillery, Burrower, Flyer, and Spitter remain incomplete catalogue roles; the presence of artwork does not activate or balance them.

...
- Leaving requires the membership and selector revisions. Authorized removal derives the Battalion from the actor's current context and requires active `MEMBER_REMOVE`; the client cannot select authority. Both retain the membership record, revoke delegations, and refuse to orphan assigned units, formation command, or active campaign participation.
...
- Rank administration requires active `RANK_MANAGE`. Rank scope and assignable permissions are server-derived; create/edit/delete/member-assignment commands use optimistic revisions, actor-scoped receipts, and Battalion-audience events, while command ranks must retain administrative recovery authority.
- Only `battalions.created_by` may transfer command. The client supplies the target plus expected Battalion/actor/target revisions; the server swaps command roles and ranks atomically, records the new creator, and returns an exact replay receipt without accepting client-authored authority.

...
**Assessed commit:** `066f8f8` plus the local Phase-0 audit/CI changes
...
| Runtime schema/fail-closed authoritative JSON | open P0 | Several Worker/DO paths cast or default invalid state. |
| Preview environment | blocked P0 | Preview D1 is a placeholder and has no deploy/migrate/smoke proof. |
| Auth retention/session operations/invite abuse controls | open P0 | No cleanup job or complete session/account surface; invitation quotas absent. |
...
| Browser E2E/accessibility/performance/security suites | open P0 | Node tests only; no production-like multi-user scenario proof. |
...
| `npm test` | local | PASS | Vitest 4.1.10; 32 files / 201 tests. |
| `WRANGLER_WRITE_LOGS=false npm run build` | local development config | PASS | Worker 470.94 kB; client JS 418.94 kB; CSS 120.83 kB. |
| `WRANGLER_WRITE_LOGS=false npm run build:production` | local production config | PASS | Compile/bundle only; no deployment. |
| Empty D1 migrations | isolated Wrangler persist directory | PASS | Migrations 0001–0007 applied; repeat reported none pending. |
...
| `npm run ci:verify:d1` | local Phase-0 CI script | PASS | Seven migrations; seven seeds twice; stable table fingerprints/counts; 112 checked tables. |
| `git diff --cached --check` | final staged Phase-0 tree | PASS | No whitespace errors. |
...
| Browser smoke/E2E | Missing | Missing | blocker |
...
| Per-route body bounds/manual validation | partial | Several routes validate, but shared runtime schemas/stored-state validation absent. |
...
| Auth/invite abuse controls and cleanup | partial/fail | Auth throttling exists; invite quotas and retention jobs do not. |
...
| Duplicate command/effect protection | partial | Several services have receipts; tactical order and journal contract incomplete. |
...
|---|---:|---:|---:|---:|---:|---:|
| Landing/auth | manual foundation only | unknown | manual foundation only | partial | unverified | missing |
...
| Tactical map/order | canvas visual only | unknown | unknown | **core route/target unavailable** | **no equivalent workflow** | missing |
...
1. Commit and run the Phase-0 application CI workflow; make it required.
2. Complete CP-002–CP-005 and reconcile documentation drift.

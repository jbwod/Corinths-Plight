...
| Canonical roster and support/aerospace systems | Partial/catalogued | D1 catalogues all thirteen canonical classes plus three companion-only classes, while generated tactical execution now covers twelve canonical allied classes. Heavy Air Transport and companion-only classes remain non-executable; no legacy class may fill that gap. |

At the current local reconciliation point, the full root suite passes **59 files / 470 tests** (the committed Phase-0 baseline was 32/201). Those results verify only the covered helpers and contracts, not the unimplemented canonical rules or release boundaries described below.

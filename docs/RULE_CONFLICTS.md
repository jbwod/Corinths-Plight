...
| Special/support actions | Partial/deferred | Load/Unload, finite-weapon Reload and Combat Medic First Aid have resolver/UI/persistence paths with known action-ledger/occupancy/cargo gaps. Scan and Deploy Drone are mechanically accepted but currently stop at events/cooldowns rather than completing their advertised state/visibility effects, so they remain release-blocked. Dig In, MASH/Medic reload, Repair, Construct, Bombardment/Funnel, Deploy/Pack Up, general Resupply, Garrison, Assault, Break Out, and other unmigrated actions remain deferred. |

...
| Special/support actions | Partial/deferred | Dig In, Load/Unload, finite-weapon Reload, Combat Medic First Aid and field resupply, Engineer vehicle Repair, Sandbag Line construction and Infantry Trench Upgrade, and Artillery Deploy/Pack Up/Bombardment have resolver/UI/persistence paths. Sandbags cost one Small Supply; the no-Supply Primary upgrade creates a Trench that preserves Dug In between connected Trench hexes. Both grant Infantry +1 Armor against fire from outside their hex. Scan and Deploy Drone remain release-blocked because their advertised state/visibility effects are incomplete. MASH, remaining construction types, Funnel, general Resupply, Garrison, Assault, Break Out, and other unmigrated actions remain deferred. |

...
| ID | Priority | State | Estimate | Slice | Dependencies | Acceptance evidence |
|---|---|---:|---:|---|---|---|
| CP-400 | P0 | IN_PROGRESS | L | Versioned scenario/map schemas, content hashes, importer/validator, map/objective/deployment/enemy/victory/reward definitions. Four authored loaders now exist: `scenario-outpost-k17-hold-relay@3`, `scenario-operation-iron-rain@1`, `scenario-operation-broken-road@1`, and `scenario-operation-night-glass@1`. Broken Road provides a 127-hex Junction 7 defence; Night Glass provides a smaller 91-hex rapid-response battle at New Carthage. Both have their own objectives, duration, insertion and reinforcement waves; unsupported content still fails closed. Terminal outcomes carry a typed reward disposition: service history is recorded while Req remains `BALANCE_REQUIRED` under RC-V5-016. General schema/import and priced rewards remain open. | CP-100, CP-200 | A campaign loads its pinned scenario; no non-K17 campaign calls the demo-state factory. |
...
| CP-601 | P1 | IN_PROGRESS | L | An operation now opens its linked live deployment planner. For linked strategic operations, validation requires one Battlegroup; commit locks exact unit/loadout snapshots, creates one tactical presence per selected unit, closes the active carrier assignment, moves the Battlegroup to the operation node as `DEPLOYED`, and activates both campaign and operation in the same D1 batch. Reinforcement plans, partial-formation presentation and strategic-order-driven bootstrap remain open. | CP-301, CP-400, CP-402 | Commit creates one tactical presence and reserves the formation atomically. |

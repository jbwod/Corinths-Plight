...
| CP-602 | P1 | IN_PROGRESS | L | Terminal tactical results already close the campaign and linked operation. Authored victory consequences now parse fail-closed and apply in the same D1 batch: Iron Rain changes Kestrel Ridge to `FRIENDLY`, opens the locked K-17 route, increments the strategic map, emits Battalion activity, and stores one `strategic_effect_receipts` row per target. Broader campaign-result ingestion through the pure strategic resolver, recovery effects and payload-collision diagnostics remain open. | CP-302, CP-402, CP-600 | Replayed result changes war state once; mismatched payload/hash fails and alerts. |

...
| ID | Priority | State | Estimate | Slice | Dependencies | Acceptance evidence |
|---|---|---:|---:|---|---|---|
| CP-400 | P0 | IN_PROGRESS | L | Versioned scenario/map schemas, content hashes, importer/validator, map/objective/deployment/enemy/victory/reward definitions. The first authored `scenario-outpost-k17-hold-relay@1` loader now builds map, objectives, enemy force, clock and four-round policy from `campaigns.map_source_key`; unsupported content fails closed and production no longer receives the demo-user K-17 state. General schema/import, deployment zones, waves, rewards and additional content remain open. | CP-100, CP-200 | A campaign loads its pinned scenario; no non-K17 campaign calls the demo-state factory. |
| CP-401 | P1 | IN_PROGRESS | M | Campaign browser, discovery/membership/withdraw/reinforce, general campaign navigation, and role policy. The authenticated campaign directory now lists only the viewer's memberships, marks authored/playable content, and drives tactical state/WebSocket/order/report URLs; production no longer selects a fixed campaign or demo user. Public discovery, join/withdraw/reinforce and campaign membership mutations remain open. | CP-206, CP-400 | No production client constant selects campaign or user; unauthorized campaign IDs remain undiscoverable. |

...
| CP-502 | P1 | IN_PROGRESS | L | Support mechanics verticals. Medic support and Engineer vehicle Repair are playable end to end. Engineers can place a V5 Sandbag Line for one Small Supply; Infantry can spend its Primary Action to convert an occupied line into a Trench, and Dug In persists while moving between connected Trench hexes. Structures persist in campaign state, render on the map, appear in reports, and grant Infantry +1 Armor against attacks from outside the hex. Artillery starts packed, has playable Deploy/Pack Up, and performs Primary Bombardment. Razor wire, tank traps, bridges, Funnel, MASH and general tactical supply remain open. | CP-500, DEC-006/DEC-009 | Each activated support action consumes the authoritative actor/resource and persists deterministic progress/effects. |

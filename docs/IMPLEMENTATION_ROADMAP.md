...
| CP-406 | P1 | DONE | S | Expose every actually executable order/action in tactical UI and remove notice-only/inert controls. The composer now offers only generated executable Hold/Advance/Rush and Attack/Reload/Load/Unload, derives action intent fields, displays finite ammo/Small Supply and paired cargo requirements, and omits catalogue-only mechanics. | CP-200, CP-404 | UI affordance matrix exactly matches server execution matrix and explains blockers. |

...
| ID | Priority | State | Estimate | Slice | Dependencies | Acceptance evidence |
|---|---|---:|---:|---|---|---|
| CP-400 | P0 | IN_PROGRESS | L | Versioned scenario/map schemas, content hashes, importer/validator, map/objective/deployment/enemy/victory/reward definitions. Three authored loaders now exist: `scenario-outpost-k17-hold-relay@3`, `scenario-operation-iron-rain@1`, and `scenario-operation-broken-road@1`. Broken Road adds a 127-hex Junction 7 defence, two objectives, five-round policy and two reinforcement waves; unsupported content still fails closed. Terminal outcomes carry a typed reward disposition: service history is recorded while Req remains `BALANCE_REQUIRED` under RC-V5-016. General schema/import and priced rewards remain open. | CP-100, CP-200 | A campaign loads its pinned scenario; no non-K17 campaign calls the demo-state factory. |
...
| ID | Priority | State | Estimate | Slice | Dependencies | Acceptance evidence |
|---|---|---:|---:|---|---|---|
| CP-600 | P1 | IN_PROGRESS | XL | The first deterministic campaign-director transition is live in the development war: an Iron Rain victory captures Kestrel Ridge, opens its route, and changes the linked Broken Road operation/campaign from announced/draft to mustering/recruiting in the same idempotent result batch. Broken Road is then joinable, supports strategic movement to Junction 7, deploys through its own insertion zone, and loads its authored five-round defence. General threat/control/supply/reinforcement selection and further operation chains remain open. | CP-302, CP-506 | Same strategic snapshot/events yield the same hashed transition; no LLM/network dependency. |

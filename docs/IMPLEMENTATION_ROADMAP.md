...
| CP-002 | P0 | IN_PROGRESS | M | Browser smoke baseline and test harness. | CP-001 | Four local Playwright canaries now cover the public auth shell, live local authenticated strategic/tactical services, a forged tactical economy field, and 390px overflow; remote CI evidence and the full production-like matrix remain open. |
...
| ID | Priority | State | Estimate | Slice | Dependencies | Acceptance evidence |
|---|---|---:|---:|---|---|---|
| CP-400 | P0 | IN_PROGRESS | L | Versioned scenario/map schemas, content hashes, importer/validator, map/objective/deployment/enemy/victory/reward definitions. The authored `scenario-outpost-k17-hold-relay@3` loader builds map, objectives, a playable ridge-approach opening enemy force, three scheduled reserve waves, clock and four-round policy from `campaigns.map_source_key`; unsupported content fails closed and production no longer receives the demo-user K-17 state. Terminal outcomes now carry a typed reward disposition: service history is recorded while Req remains `BALANCE_REQUIRED` under RC-V5-016. General schema/import, priced rewards and additional content remain open. | CP-100, CP-200 | A campaign loads its pinned scenario; no non-K17 campaign calls the demo-state factory. |
...
| CP-406 | P1 | DONE | S | Expose every actually executable order/action in tactical UI and remove notice-only/inert controls. The composer now offers only generated executable Hold/Advance/Rush and Attack/Reload/Load/Unload/Heal, derives action intent fields, displays finite ammo/Small Supply/Medical Supply and paired cargo requirements, and omits catalogue-only mechanics. | CP-200, CP-404 | UI affordance matrix exactly matches server execution matrix and explains blockers. |
...
| ID | Priority | State | Estimate | Slice | Dependencies | Acceptance evidence |
|---|---|---:|---:|---|---|---|
| CP-500 | P1 | IN_PROGRESS | L | Movement contention, terrain/elevation/roads/rivers, facing, cover/high ground/smoke, structures and remembered intelligence. Terrain Advantage is live: authoritative map elevation grants ground-only +1 before FS cap/mitigation. Dig In is also playable end to end: full-Speed Hold action, persistent `+2 Defense`, rear exclusion, movement cancellation, reports and UI. Dynamic cover Armor, smoke, structures, unpinned movement-cost defaults and remembered intelligence remain open. | CP-200, CP-400, decisions DEC-006/DEC-007 | Pure permutation/golden tests and event-time projections cover every activated terrain rule. |
| CP-501 | P1 | IN_PROGRESS | L | Multiweapon, Rapid Fire, arcs/domain, rear exclusions, subsystem damage/repair, indirect spotting, ammo/cooldown and simultaneous reports. Governed tags apply Rapid Fire versus Horde and constrain rear effects to ground vehicles/infantry while excluding air. The governed durability profile drives persistent natural-5/6 subsystem failures after simultaneous attacks. Multiweapon, forward firing arcs, high ground/Evasive and expanded calculation provenance remain open. | CP-200, applicable decisions | Reports show auditable calculation; no air flank, overfire, hidden-target or cooldown defect. |

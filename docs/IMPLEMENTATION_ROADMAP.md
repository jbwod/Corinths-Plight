...
| ID | Priority | State | Estimate | Slice | Dependencies | Acceptance evidence |
|---|---|---:|---:|---|---|---|
| CP-400 | P0 | IN_PROGRESS | L | Versioned scenario/map schemas, content hashes, importer/validator, map/objective/deployment/enemy/victory/reward definitions. The authored `scenario-outpost-k17-hold-relay@3` loader builds map, objectives, a playable ridge-approach opening enemy force, three scheduled reserve waves, clock and four-round policy from `campaigns.map_source_key`; unsupported content fails closed and production no longer receives the demo-user K-17 state. General schema/import, deployment zones, rewards and additional content remain open. | CP-100, CP-200 | A campaign loads its pinned scenario; no non-K17 campaign calls the demo-state factory. |
...
| CP-502 | P1 | IN_PROGRESS | L | Support mechanics verticals. Combat Medic First Aid and field resupply are playable end to end: a deployed Medic selects wounded friendly Infantry in base contact, spends one Medical Supply, resolves the V5 D6/current-FS cap, and may later spend one Small Supply to restore Medical Supply to its current FS. Both paths emit dedicated events and persist health/supplies. Engineer, construction, artillery, structures, MASH and general tactical supply remain open. | CP-500, DEC-006/DEC-009 | Each activated support action consumes the authoritative actor/resource and persists deterministic progress/effects. |

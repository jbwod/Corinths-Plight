...
| Multi-planet living war | partial | High | One development strategic fixture and a supported manual-resolution slice exist. Iron Rain now connects its Operation briefing to a server-authoritative one-Battlegroup deployment commit, applies authored Kestrel control and route-unlock consequences exactly once, then returns survivors and the participating Battlegroup to an explicit recovery state. Production content, strategic travel, reinforcement/recovery actions and additional worlds remain open. | CP-302–CP-305/CP-600–CP-604. |

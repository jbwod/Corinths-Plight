...
| Unit/contract tests | Pass | `npm test`: 59 files, 452 tests, Vitest 4.1.10. |
...
|---|---|---:|---:|---|---|
| AUD-CAT-001 — split gameplay truth | P0 | partial | High | A generated, hash-verified `v5-core-curated@2` catalogue now materializes seven playable allied unit/weapon definitions and executable action/order grammar for onboarding, Force catalogue projection, deployment hydration, tactical UI lookup, submission and resolution. D1-only IFV/VTOL/HAT records fail before execution. D1 still stores the legacy `@1` relational identity for instance FKs and validation, and immutable `@2` D1 publication remains open. | CP-200/CP-201 |
...
| AUD-RULE-007 — cargo/action ledger gaps | P1 | partial | Medium | Ground Load/Unload now uses the generated action ledger, exact governed Logi/Light Vehicle unit-and-Supply capacity, paired consent, carrier-position coupling, occupancy-safe unload and persistent manifests. Packed Artillery can hitch to one Logi tow point, move with it and unhitch without consuming cargo slots. Specialist HAT/airdrop validation remains open. | CP-503/CP-505 |

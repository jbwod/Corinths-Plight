...
| Unit/contract tests | Pass | `npm test`: 59 files, 477 tests, Vitest 4.1.10. |
...
| Browser/a11y/performance tests | Browser baseline partial | `npm run test:browser`: 9/9 pass for the signed-out auth/enlist shell, authenticated live local strategic/tactical reads without showcase fallback, strategic Disembark, strategic deployment authorization through tactical snapshot commit, forged tactical economy rejection, Heavy Air Transport composer, executable-action composer and planning-layer coverage, and 390px document overflow. Full onboarding/game E2E, Axe/manual accessibility, performance and load remain CP-800/CP-702/CP-802. |
...
| Tactical realtime/report API | partial | High | Campaign DO report endpoint; `src/components/CampaignReports.tsx` | Round detail is visible and grouped. Terminal reports also project acknowledged Battalion-visible strategic effect receipts so the player can see the node, route and operation changes caused by victory and return directly to the Galactic map. Socket invalidations are per-viewer, identifier-free and include bounded events-after-cursor reconnect catch-up. Reports still use current-state rather than event-time redaction, and playback/export remain CP-403/CP-700. |

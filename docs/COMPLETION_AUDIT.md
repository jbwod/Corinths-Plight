...
|---|---|---:|---:|---|---|
| AUD-CAT-001 — split gameplay truth | P0 | partial | High | A generated, hash-verified `v5-core-curated@2` catalogue/public projection and CP-201 execution adapter now prevent D1-only Logi/IFV/VTOL/HAT records from reaching the five-class `@1` handler. Tactical order submission, hydration and resolution use the generated action/order grammar and hash; acquisition availability is separate from execution of already-owned foundation units. Unit/weapon materialization, D1 publication and Force/UI consumers still depend on compatibility data. | CP-200/CP-201 |

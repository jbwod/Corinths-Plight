...
**Commit:** `72b46cc` (`codex/phase0-release-baseline`) plus the current uncommitted implementation slices; no release commit is claimed

**Authority:** `docs/GAME_COMPLETION_GOAL.md`

**Verdict:** **Not release ready**

The repository is a healthy Cloudflare-native foundation with production passwordless identity, guided onboarding, persistent force/loadout services, deployment-plan commits, usable strategic commands, a deterministic tactical core, and a local admin-only Game Master/map-authoring vertical slice. It is not yet the advertised living cooperative war. Production-facing clients now fail closed instead of presenting development showcase state, but several deferred systems and production acceptance gates remain. Custom maps publish and bootstrap exact-pinned recruiting campaigns locally, and new custom scenario content pins the bounded `game-master-skirmish@1` terminal policy plus `public-v1-economy@1`; the path still lacks recorded production evidence and standalone-custom strategic placement. Rules instance storage and D1 publication still retain legacy identity alongside the generated runtime catalogue.
...
|---|---|---|
| Seed/content validator | Pass | `npm run seed:check`: 49 definitions, 43 active, 225 SQL definitions, 13 canonical and 16 Phase-2 allied classes, 7 enemy roles, 4 Phase-3 operations, 9 equipment effects, 6 deployment methods, 8 source hashes. |
| TypeScript | Pass | `npm run typecheck`. |
| ESLint | Pass | `npm run lint`. |
| Unit/contract tests | Pass | `npm test`: 106 files, 849 tests, Vitest 4.1.10. |
| Worker/client build | Pass | `npm run build`; Worker 2,053.02 kB, client JS 1,920.16 kB, CSS 218.38 kB. The explicit manifest bundles 29 runtime tactical sheets totalling 7.77 MiB and excludes every superseded/quarantined mech revision; the post-build verifier checks the emitted hashes. Wrangler's sandboxed debug-log write warns but the build exits successfully; bundle/performance budgets remain CP-802 work. |
| Production-mode build | Pass | `WRANGLER_WRITE_LOGS=false npm run build:production`. |
| Empty D1 migration replay | Pass | All twenty-two migrations through `0022_game_master_skirmish_policy.sql` applied in isolated Wrangler state; nine seeds replayed twice across 130 application tables with integrity/FK clean. |
| Repeat seed replay | Pass | All nine seeds applied twice. |
| D1 integrity | Pass | SQLite `integrity_check=ok`; `foreign_key_check` empty; 22 migration records and 130 application tables. |
...
| Browser/a11y/performance tests | Browser baseline partial | `npm run test:browser` passes 20/20 locally, including public auth, Battalion/ship/force persistence, the Game Master generate→publish→exact-pinned campaign workflow, strategic authorisation, persisted 311-hex Iron Rain bootstrap, four-round tactical resolution/replay, keyboard map operation, and 390px overflow. Axe/manual accessibility, performance, load and real multi-account/production evidence remain CP-800/CP-702/CP-802. |
...
| Guided Battalion join/create/invite, active switch, leave/remove, rank/command administration, starter unit and tour | implemented foundation | High | `worker/routes/onboarding.ts`; `worker/routes/battalion-admin.ts`; `src/components/GuidedOnboarding.tsx`; `src/components/strategic/BattalionView.tsx` | Rank create/edit/permission assignment/member assignment/unused deletion, atomic creator-command transfer, and departure recovery are live locally. Invite revocation, opt-out and production abuse evidence remain CP-104/CP-206. |
| Force list/detail/history, unit/equipment purchase and loadout | partial/playable | High | Force/equipment routes and services | Unit purchase now hands directly into the live loadout. Eligible Store purchases atomically debit Req/create owned inventory, auto-place into a compatible slot, preview the authoritative combat package, and persist only through the versioned loadout command. Generated availability blocks known non-functional equipment. Item breadth, consumable recovery and local fallback removal remain CP-200–CP-208. |
| Deployment planning and commit | partial/playable | High | `worker/routes/deployment.ts`; `worker/services/deployment.ts`; `src/components/DeploymentPlanner.tsx` | Production uses the authenticated campaign selection and exact authored/custom scenario pin. Committed force/loadout snapshots enter both authored `@3` and published custom `@2` runtimes; wider strategic lift, tactical extraction, and partial-formation UX remain CP-400/CP-401/CP-601 work. |
| Tactical map and executable composer | partial/playable | High | `src/App.tsx`; `src/components/HexMap.tsx`; `src/tactical-visuals.ts`; `src/tactical-terrain.ts`; generated tactical grammar; Campaign DO/resolver | Authenticated authored campaigns expose generated Hold/Advance/Rush and migrated support actions with server-derived costs/blockers. Routes resolve on quarter-distance simultaneous timing, persist legal prefixes, and report hostile/capacity stops. Attack declares one target and the server fires every eligible fitted weapon with per-weapon report evidence. Cooperative readiness names missing Allied orders; submitted audience-safe Allied intents render as outlined maneuver/assault/support/fortify/hold corridors while drafts stay private. Four versioned operations now preserve their former authored cores inside deterministic connected, irregular theatre footprints. The renderer auto-fits land plus three cosmetic water-depth rings, derives roads/rivers from authoritative edges, varies only the four existing terrain families, and keeps unknown hexes neutral. A fog-safe keyboard cursor announces projected hex facts and selects routes through the same callback. Same-hex units use deterministic combined-arms formations, side-keyed sprite grading, per-owner base colours and facing-rotated pointers without changing authoritative position/capacity. The local capture/browser run is renderer evidence, not real multi-account proof. Broader combined-arms rules, scenario-balance approval, multi-account browser proof and a complete semantic route/target alternative remain CP-500–CP-507/CP-701–CP-702. |
...
| Strategic command/Battalion/ship/map/operation reads | implemented read model | High | `worker/routes/strategic.ts`; `src/strategic/api.ts`; `src/components/StrategicWorkspace.tsx` | Primary-ship identity has a separate live mutation route. Production required-read failures render an unavailable state instead of showcase formations/operations; the showcase adapter remains development-only. Partial optional projections, production world publication, and separate strategic acceptance remain CP-305. |
...
| Game Master campaign control and map authoring | local partial | High | `worker/routes/game-master.ts`; `worker/game-master-authoring.ts`; `worker/game-master-runtime.ts`; `src/components/GameMasterConsole.tsx`; migrations `0019`–`0022` | An active explicit global grant authorizes live state/audit, clock, objective, governed-enemy spawn, pause/resume, ordinary resolve, and exact `game-master-recovery@1` exceptional-correction commands through versioned actor-scoped receipts. Five deterministic strict `@2` presets edit/save/reopen/export/publish immutable revisions and create exact planet-pinned recruiting campaigns through normal deployment/DO bootstrap. New custom scenario content `@2` pins the bounded skirmish closure and published reward policy; legacy `@1` remains fail-closed. Large custom battlefields use verified chunked DO state. Recovery is server-authored, paused-from-planning, destroyed-target-only, and reconciles persistent Allied state; it is not ordinary V5 repair. Standalone-custom strategic placement/recovery, grant lifecycle/MFA, wider director controls, cross-store diagnostics, and public-release evidence remain CP-405/CP-400/CP-603. |
...
| AUD-SCEN-001 — unauthored or unversioned tactical campaigns inherited current content | P0 | resolved for current authored/custom content | High | Persistent initialization and discovery require either one of five immutable authored pairs or an exact published custom map revision/hash. D1, source document, stored Durable Object identity/version, and the custom application/reward tuple are checked before use; legacy NULL pins, legacy custom `@1`, draft/unavailable revisions, hash mismatches, and stored mismatches fail closed. New custom `@2` maps bootstrap through committed Allied deployment and use the bounded skirmish closure. General schema/import and an explicit legacy-campaign disposition remain CP-400/CP-401/CP-405 work. | CP-400/CP-401/CP-405 |
...
| AUD-GM-001 — director controls and custom maps | P1 / release-sensitive | partial | High | Global grant checks, trusted-header stripping, versioned/idempotent campaign and authoring commands, private audits, five deterministic presets, complete `@2` terrain/feature mechanics, strict import/export, immutable publication, exact-pinned recruiting bootstrap, `game-master-skirmish@1` closure/rewards, and the exact `game-master-recovery@1` exceptional correction exist locally. Recovery is not canonical V5: it requires paused-from-planning state, a destroyed deployment at a legal hex, and server-derived restoration, with persistent Allied D1 reconciliation and unit history. Standalone-custom strategic placement, grants/MFA/rate/alerts, early-rejection auditing, cross-store diagnostics/recovery controls and public-release evidence remain open. | CP-405/CP-400/CP-603 |
...
| AUD-RULE-005 — unactivated terrain defaults execute | P1 | resolved for custom-map `@2`; legacy partial | High | `corinth-custom-map-terrain-application@2` explicitly governs every current custom biome/point/edge profile, including water/mountain blockers, amphibious exceptions, airborne state, quarter-step terrain costs, cover/LOS/capacity, Road/Path/River/Bridge/Wall and all six point features. Older authored scenario defaults remain separately pinned and are not retroactively reconciled. | CP-400/CP-500 |
...
| AUD-UI-001 — authoritative-looking local fallbacks | P0 | partial/production substitution closed | High | Production campaign loading, Forces, and Strategic surfaces now show loading/error/empty states and never render the retained development showcase as persistent truth. Development still keeps local tactical/force/strategic fixtures, and production browser evidence plus hardening/removal of unused in-memory seed state remain CP-208/CP-305/CP-703 work. | CP-208/CP-305/CP-703 |
| AUD-UI-002 — hard-coded production context | P0 | partial | High | Tactical campaign/deployment selection comes from authenticated directories, production starts without a default campaign ID, and demo headers are development-only. Fixed Resolute/33rd/fixture identities remain confined to development showcase/reference adapters; production truth-boundary and authenticated browser evidence remain required. | CP-208/CP-401/CP-703 |
| AUD-UI-003 — client order-timeline adapter | P0 | resolved for authoritative mutation; presentation partial | High | Order save now refetches authoritative campaign state. The Orders timeline maps those projected server orders into `ui:`-prefixed display records but does not insert them into `campaign.events` or persist them as journal truth. A dedicated order-timeline DTO would remove the remaining type/presentation ambiguity. | CP-403/CP-406 |
| AUD-UI-004 — equipment selection does not mutate purchase | P1 | resolved | High | The false initial-equipment checkboxes were removed. Unit purchase opens its real default loadout; equipment is acquired through the owner-scoped Req endpoint, returned as an owned inventory instance, automatically proposed in a compatible slot, authoritatively previewed, and installed only by the exact-once loadout commit. | CP-204 |
...
| AUD-QA-001 — browser baseline is not the release matrix | P0 | partial | High | Twenty local Playwright journeys and CI artifact wiring now exist, but there is no production-like multi-user run, Axe/screen-reader evidence, load or soak suite. Tactical canvas still lacks a semantic route/target alternative. | CP-002/CP-702/CP-800/CP-802 |
...
| `GET /api/game-master/session` | Global Game Master capability projection | implemented local; no production grant/default deployment |
| `GET /api/game-master/campaigns`, `GET /api/game-master/campaigns/:id/state` | Live campaign registry and full trusted Game Master projection | implemented local |
| `PATCH/POST /api/game-master/campaigns/:id/*` | Clock, objective, pinned-enemy spawn, pause/resume/resolve and exact exceptional recovery | implemented local subset with versioned actor-scoped receipts/audit; recovery is paused-from-planning and destroyed-target-only |
| `GET/POST /api/game-master/maps`, `GET /api/game-master/maps/:id` | List/save/reopen strict canonical versioned map drafts | implemented local |
| `POST /api/game-master/maps/:id/publish` | Exact current-document mechanics and revision/hash publication check | implemented local for governed `@2` documents |
| `POST /api/game-master/campaigns` | Create exact published map-revision-pinned campaign | implemented `RECRUITING`; deterministic insertion zone and normal deployment/runtime bootstrap |
| `GET /api/game-master/audit` | Combined bounded campaign/authoring audit projection | implemented local |
...
No general public campaign-creation, profile/settings/session-management, complete refit/construction, notification, rules-publication, recovery, or release-grade operator reconciliation route exists. Game Master grants also have no public administration endpoint.
...
| `POST /resolve` | Trusted Game Master manual tactical resolve | receipt-idempotent/versioned command; broader resolution journal remains partial |
| `PATCH /clock` | Trusted Game Master clock update | receipt-idempotent/versioned and D1 registry-projected |
| `POST /game-master/commands` | Objective/spawn/pause/resume/resolve/revive command family | implemented local subset; revive applies only `game-master-recovery@1` under the global-GM and paused-from-planning boundary |
...
|---|---:|---|
| Landing registration/login/verification/logout | implemented foundation | Server-backed auth flow. The signed-out copy now labels the strategic capture illustrative and names the current authenticated-force/tactical/Game-Master game-test scope; legal/support/accessibility and public-release claims remain gated. |
...
| Tactical campaign selection | implemented foundation | Production starts without a default campaign ID, loads the authenticated campaign directory, selects only an enterable membership, and shows loading/error/directory states until that exact live projection arrives. `outpost-k17` remains a development-only default. |
...
| Game Master console | partial/local | Navigation is hidden unless `/api/game-master/session` reports an explicit global grant. The accessible console lists live campaigns, edits clocks/objectives, spawns governed enemies, pauses/resumes/resolves, applies the confirmed `game-master-recovery@1` exceptional correction, reads audit, and generates/imports/edits/saves/reopens/exports/publishes five-preset maps. Published maps create exact-pinned recruiting campaigns whose new `@2` content uses the bounded skirmish/reward policy. Recovery is visibly exceptional and server-governed; standalone-custom strategic placement remains open. |
...
| Forces browse/inspect/readiness/purchase/loadout | partial/playable | Owner-scoped live routes support unit purchase → immediate quartermaster handoff → equipment Req purchase → compatible-slot proposal → server-derived combat preview → versioned commit. Reserve/ship-facility refits no longer require a fixed operation ID. Demo presentation fallback and broader item lifecycle remain. |
| Forces initial-equipment checkboxes | removed | Unit acquisition no longer implies bundled equipment. The live quartermaster is the only Store ownership/install path. |
| Forces identity/history | partial/playable | An owner can edit an active unit's name, callsign and service description from the inspection view. The server enforces optimistic versioning and idempotency, records `RENAMED` history, and rejects memorial mutations; the live browser proof reads the persisted identity and history back from D1. Broader recovery/repair lifecycle remains CP-203. |
...
| Unit/equipment gameplay imagery | integrated local slice | Stable registries cover all 29 catalogued non-orbital unit classes and all 105 Store equipment definitions. Portraits are used across Forces, deployment and tactical inspection; equipment images are used in requisition/loadout/inspection and ship modules. The tactical allowlist selects one six-frame sheet per unit; the newly generated light-v3, medium-v4 and heavy-v3 mechs pass strict roof-only/nadir QA, while the other 26 sheets remain provisional. Mech source/packed hashes and every alternate/quarantined disposition are recorded and tested; equivalent provenance/QA for the other tactical families is still required. Semantic glyph fallback and reduced-motion states remain available. Final public legal review and unrelated planet/ship/legacy asset coverage remain open. |
...
Fresh replay produced 130 application tables, excluding SQLite/Cloudflare internals and `d1_migrations`. The grouped names below are the reproducible schema inventory. Group status describes the strongest production workflow touching the family; individual gaps are called out afterward.
...
| Game Master authority/audit | `game_master_grants`, `game_master_command_receipts`, `game_master_audit_events` | local explicit-grant and audited campaign-command workflow; no grant administration/MFA/production deployment |
| Game Master map authoring/runtime | `game_master_maps`, `game_master_map_revisions`, `game_master_authoring_receipts`, `game_master_authoring_audit_events`, `game_master_campaign_scenarios` | local draft edit/save/reopen/export, immutable publication, exact custom campaign/scenario pin, and recruiting/runtime bootstrap |
...
CP-004 now also records the local `0019`–`0022` Game Master/map-authoring/runtime slice, the explicit global-grant boundary, immutable custom publication/bootstrap, the exact version-2 skirmish/reward policy, chunked DO state, cross-store audit limitation, and the owner-approved custom-map terrain application. The generated catalogue still distinguishes 13 canonical playable allied classes from 16 blocked companion classes and the 72 canonical Markdown conflicts from 12 obsolete D1 IDs. Documentation agreement does not close the underlying implementation findings.

## Release decision

Current release decision: **NO-GO**.

The owner has separately authorized a private production game-test deployment and waived preview plus a pre-deployment backup for that operation. That test is not a public release: its claim is limited to authenticated onboarding, Battalion/force/deployment, tactical campaign/report/recovery, and explicitly granted Game Master workflows. Strategic Command/Galactic and Ship remain outside the production test-ready scope. The waiver does not close AUD-OPS-001/AUD-OPS-002 or supply RPO/RTO evidence.

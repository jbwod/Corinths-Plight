...
| Unit/contract tests | Pass | `npm test`: 96 files, 736 tests, Vitest 4.1.10. |
...
| Empty D1 migration replay | Pass | All eighteen migrations through `0018_campaign_scenario_content_pins.sql` applied in isolated Wrangler state; nine seeds replayed twice across 122 tables with integrity/FK clean. |
| Repeat seed replay | Pass | All nine seeds applied twice. |
| D1 integrity | Pass | SQLite `integrity_check=ok`; `foreign_key_check` empty; 18 migration records and 122 application tables. |
...
| Browser/a11y/performance tests | Browser baseline partial | The prior `npm run test:browser` baseline remains 18/18. The affected composer and map-layer journeys remain green; on 2026-08-14 a clean isolated strategic-authorisation-to-persisted-V3-tactical journey also passed with 311 Iron Rain hexes and keyboard traversal/selection. Full onboarding/game E2E, Axe/manual accessibility, performance, load and real multi-account visual evidence remain CP-800/CP-702/CP-802. |
...
| AUD-SCEN-001 — unauthored or unversioned tactical campaigns inherited current content | P0 | resolved for current authored content | High | Persistent initialization and campaign discovery require one of five supported `map_source_key` + immutable `scenario_content_key` pairs. D1 and stored Durable Object scenario identity/version are checked before use; legacy NULL pins, unavailable versions, and mismatches fail closed without storage mutation. Migration `0018` deliberately has no backfill or automatic upgrade. A general scenario importer, content hash publication, and an operator policy for legacy campaigns remain CP-400/CP-401 work. | CP-400/CP-401 |

...
| Unit/contract tests | Pass | `npm test`: 53 files, 398 tests, Vitest 4.1.10. |
...
| Tactical map and executable composer | partial/playable | High | `src/App.tsx`; `src/components/HexMap.tsx`; generated tactical grammar; Campaign DO/resolver | Authenticated authored campaigns expose generated Hold/Advance/Rush and migrated support actions with server-derived costs/blockers. Routes resolve on quarter-distance simultaneous timing, persist legal prefixes, and report hostile/capacity stops. Broader combined-arms rules and accessible map alternative remain CP-500–CP-507/CP-702. |

...
| Guided Battalion join/create/invite, active switch, leave/remove, starter unit and tour | implemented foundation | High | `worker/routes/onboarding.ts`; `src/components/GuidedOnboarding.tsx`; `src/components/BattalionRecruitmentPanel.tsx` | Revisioned leave/remove and active-context recovery are live locally. Rank administration, ownership transfer, invite revocation, opt-out and production abuse evidence remain CP-104/CP-206. |

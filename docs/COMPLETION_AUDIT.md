...
| Browser/a11y/performance tests | Browser baseline partial | `npm run test:browser`: 13/13 pass for the signed-out auth/enlist shell, authenticated live local strategic/tactical reads without showcase fallback, campaign join/staging/withdrawal with exact replay protection, persistent unit identity/history editing, an authoritative Reserve loadout preview/commit, strategic Disembark, resolved Large-Supply logistics, strategic deployment authorization through tactical snapshot commit, forged tactical economy rejection, Heavy Air Transport composer, executable-action composer, four-round report playback, planning layers, and 390px document overflow. Full onboarding/game E2E, Axe/manual accessibility, performance and load remain CP-800/CP-702/CP-802. |
...
| Guided Battalion join/create/invite, active switch, leave/remove, rank administration, starter unit and tour | implemented foundation | High | `worker/routes/onboarding.ts`; `worker/routes/battalion-admin.ts`; `src/components/GuidedOnboarding.tsx`; `src/components/strategic/BattalionView.tsx` | Rank create/edit/permission assignment/member assignment/unused deletion and departure recovery are live locally. Ownership transfer, invite revocation, opt-out and production abuse evidence remain CP-104/CP-206. |

...
| Unit/contract tests | Pass | `npm test`: 59 files, 470 tests, Vitest 4.1.10. |
...
| Tactical map and executable composer | partial/playable | High | `src/App.tsx`; `src/components/HexMap.tsx`; generated tactical grammar; Campaign DO/resolver | Authenticated authored campaigns expose generated Hold/Advance/Rush and migrated support actions with server-derived costs/blockers. Routes resolve on quarter-distance simultaneous timing, persist legal prefixes, and report hostile/capacity stops. Attack declares one target and the server fires every eligible fitted weapon with per-weapon report evidence. Broader combined-arms rules and accessible map alternative remain CP-500–CP-507/CP-702. |
...
| Tactical realtime/report API | partial | High | Campaign DO report endpoint; `src/components/CampaignReports.tsx` | Round detail is visible and grouped. Socket invalidations are now per-viewer, identifier-free and include bounded events-after-cursor reconnect catch-up. Reports still use current-state rather than event-time redaction, and index/playback/export remain CP-403/CP-700. |
...
| Multi-planet living war | partial/playable development vertical | High | The development theatre contains the three-operation Corinth chain plus independent Cold Horizon on Corinth II. A real local run rejected Raven's deployment at Corinth, moved Resolute through a five-round two-edge route, persisted the ship at `location-corinth-ii`/`planet-corinth-ii`, committed two field-ready Raven snapshots, and booted the five-round 127-hex tactical scenario. A separate live round trip proves Disembark → reserve-node persistence → Embark → primary-ship persistence for every Raven member. Production content, travel supply, branching outcomes and additional worlds remain open. | CP-302–CP-305/CP-600–CP-604. |
...
| AUD-RULE-007 — cargo/action ledger gaps | P1 | partial | Medium | Ground Load/Unload and generic VTOL cargo use the generated action ledger, governed mutually-exclusive capacity, paired consent, carrier-position coupling, occupancy-safe unload and persistent manifests. Packed Artillery can hitch to one Logi tow point, move with it and unhitch without consuming cargo slots. Specialist HAT/airdrop validation and carrier-destruction adjudication remain open. | CP-503/CP-505 |
...
| Bomber | yes | yes, partial | partial/playable | Speed-6 Atmo Flight, hostile-ground passage, D6 Range-0 one-shot ordnance, no-ground-spotting and route-bound fly-over targeting execute through generated authority. Landing/rearm remain deferred. |

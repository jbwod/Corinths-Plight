...
That node/Battlegroup transition applies only when the terminal campaign has a linked strategic operation. The generic result path for a standalone Game Master custom campaign still closes deployments, preserves destroyed units, returns survivors to unlocated `RESERVE`, and unlocks loadouts, but it does not invent an operation node, carrier transition, or `RECOVERING` Battlegroup state. Supplying that placement is explicit CP-603 work rather than an implicit consequence of `game-master-skirmish@1`.
...
These strategic surfaces remain outside the owner-approved private production game-test scope. They may continue to exist for development and future validation, but Strategic Command/Galactic and Ship must not be presented as test-ready production gameplay until their separate acceptance gates are met.

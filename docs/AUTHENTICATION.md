...
Migration `0008` was separately replayed from an empty local D1 before deployment. The current shared gate passes all 22 migrations, ten seeds twice, typecheck, lint, 107 Vitest files / 855 tests, and the production build. Production is now through `0022`; preview and broader operational evidence remain pending.

The latest recorded production deployment is version `e88137dd-19ae-4bf5-8050-3accf8f14343`, migrated through `0022`; it includes the passwordless boundary, verified `register@corinth.qnetica.com.au` sender, both managed Worker secrets, and the hourly retention/invitation schedule. Current live checks passed for health, signed-out session availability, SPA delivery, and security headers. Historical Resend and verification-navigation checks still apply, but do not replace preview, browser-matrix, or ongoing operational evidence.

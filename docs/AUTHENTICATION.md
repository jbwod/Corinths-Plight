...
Production version `241d0fac-60ea-47b0-a022-c57b210a1a67` is deployed with migration `0006`, the verified `register@corinth.qnetica.com.au` sender, and both managed Worker secrets. Live checks passed for health, signed-out session availability, same-origin rejection, desktop/mobile UI rendering, Resend delivery, and top-level `/api/auth/verify` navigation reaching the Worker. The initial asset-routing incident left challenges safely `SENT` and created no User or Session; the API-first routing fix and explicit confirmation POST were deployed before any production account was created.

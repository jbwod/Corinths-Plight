...
function render(rows: StoreRow[]): string {
  const publicMechWeaponRows = new Set([59, 60, 63, 64, 65]);
  const publicImplemented = (row: number) => row === 16 || row === 68 || publicMechWeaponRows.has(row);
  ...
  const definitionTuples = newRows.map((row) => {
    ...
    const powerArmourImplemented = publicImplemented(row.row);
    const storeCatalogue = {
      ...
      effectStatus: publicMechWeaponRows.has(row.row) ? "PUBLIC_V1_MECH_WEAPON_HANDLER" : powerArmourImplemented ? "PUBLIC_V1_POWER_ARMOUR_HANDLER" : "SOURCE_EXACT_HANDLER_DEFERRED",
    };
    return `  (${sql(row.id)}, ${sql(rulesetId)}, ${sql(row.name)}, ${sql(category(row.row))}, ${sql(slotTypes(row.row)[0].toLowerCase())}, ${price}, 0, ${sql(powerArmourImplemented ? "active" : "experimental")}, 'The Store row ${row.row}', ${sql(powerArmourImplemented ? "Executable through the public-v1 Power Armour resolver." : "Source-exact catalogue entry; execution remains gated until its effect handler is implemented.")}, ${json({ storeCatalogue })})`;
  });
  const cataloguePatches = rows.map((row) => `UPDATE equipment_definitions
SET definition_json = json_patch(definition_json, ${json({ storeCatalogue: { storeRow: row.row, sourceName: row.sourceName, sourceSlot: row.slotText, rulesText: row.rulesText, unitAccessText: row.accessText, effectStatus: publicMechWeaponRows.has(row.row) ? "PUBLIC_V1_MECH_WEAPON_HANDLER" : row.row === 16 || row.row === 68 ? "PUBLIC_V1_POWER_ARMOUR_HANDLER" : existingIds.has(row.row) ? "SEE_IMPLEMENTATION_OVERLAY" : "SOURCE_EXACT_HANDLER_DEFERRED" } })})
WHERE id = ${sql(row.id)} AND ruleset_id = ${sql(rulesetId)};`);
  ...
  const overlays = newRows.map((row) => {
    ...
    if (publicImplemented(row.row)) {
      return `  ('${kind}', ${sql(row.id)}, ${sql(rulesetId)}, 'IMPLEMENTED', 'PUBLISHED', 'AVAILABLE', 1, 1, NULL, ${sql(sourcePath)}, ${sql(`row ${row.row}`)}, ${json({ sourceExact: true, storeRow: row.row, applicationProfileId: "public-v1-power-armoured-infantry@1" })})`;
    }
...
-- Approved public-v1 conversion for the source Store mech weapon family. The
-- original FS prose remains in definition_json; runtime dice are explicit and
-- versioned here so Medium/Heavy Mechs never infer attacks from mutable FS.
INSERT INTO weapon_definitions (
  ...
  ('weapon-mech-heavy-machine-public-v1', '${rulesetId}', 'Heavy Machine Weapon', 1, 4, 0, 0, 1, 4, NULL, 0, 'active', 'The Store row 59 + companion-v1-mechs@1', 'Four-round magazine; Supply Point reload.', '{"tags":["DIRECT","MECH_WEAPON","BURST_FIRE"]}'),
  ('weapon-mech-autocannon-public-v1', '${rulesetId}', 'Mech Autocannon', 1, 6, 0, 2, 2, 2, NULL, 0, 'active', 'The Store row 60 + companion-v1-mechs@1', 'Two-round magazine; Supply Point reload.', '{"tags":["DIRECT","MECH_WEAPON","AUTOCANNON"]}'),
  ('weapon-mech-light-laser-public-v1', '${rulesetId}', 'Light Laser', 1, 4, 0, 0, 1, 3, NULL, 0, 'active', 'The Store row 63 + companion-v1-mechs@1', 'Three shots before cooling; Supply Point reset.', '{"tags":["DIRECT","MECH_WEAPON","ENERGY","COOLING_3"]}'),
  ('weapon-mech-medium-laser-public-v1', '${rulesetId}', 'Medium Laser', 1, 6, 0, 0, 2, 2, NULL, 0, 'active', 'The Store row 64 + companion-v1-mechs@1', 'Two shots before cooling; Supply Point reset.', '{"tags":["DIRECT","MECH_WEAPON","ENERGY","COOLING_2"]}'),
  ('weapon-mech-large-laser-public-v1', '${rulesetId}', 'Large Laser', 1, 8, 0, 0, 3, 1, NULL, 0, 'active', 'The Store row 65 + companion-v1-mechs@1', 'One shot before cooling; Supply Point reset.', '{"tags":["DIRECT","MECH_WEAPON","ENERGY","COOLING_1"]}')
ON CONFLICT(id, ruleset_id) DO UPDATE SET
  ...
  ammo_capacity=excluded.ammo_capacity, definition_status=excluded.definition_status,
  source=excluded.source, notes=excluded.notes, definition_json=excluded.definition_json;

UPDATE equipment_definitions SET definition_status='active'
WHERE ruleset_id='${rulesetId}' AND id IN ('equipment-mech-heavy-machine-weapon','equipment-mech-autocannon','equipment-mech-light-laser','equipment-mech-medium-laser','equipment-mech-large-laser');
...
INSERT INTO equipment_effect_definitions (
  ...
  ('equipment-mech-heavy-machine-weapon','${rulesetId}',0,'WEAPON_GRANT','{"type":"WEAPON_GRANT","weaponId":"weapon-mech-heavy-machine-public-v1"}','${sourcePath}','row 59'),
  ('equipment-mech-autocannon','${rulesetId}',0,'WEAPON_GRANT','{"type":"WEAPON_GRANT","weaponId":"weapon-mech-autocannon-public-v1"}','${sourcePath}','row 60'),
  ('equipment-mech-light-laser','${rulesetId}',0,'WEAPON_GRANT','{"type":"WEAPON_GRANT","weaponId":"weapon-mech-light-laser-public-v1"}','${sourcePath}','row 63'),
  ('equipment-mech-medium-laser','${rulesetId}',0,'WEAPON_GRANT','{"type":"WEAPON_GRANT","weaponId":"weapon-mech-medium-laser-public-v1"}','${sourcePath}','row 64'),
  ('equipment-mech-large-laser','${rulesetId}',0,'WEAPON_GRANT','{"type":"WEAPON_GRANT","weaponId":"weapon-mech-large-laser-public-v1"}','${sourcePath}','row 65')
...
SET implementation_status='IMPLEMENTED', requisition_status='PUBLISHED', availability_status='AVAILABLE',
    ...
    overlay_json=json_patch(overlay_json, '{"applicationProfileId":"companion-v1-mechs@1","effect":"FITTED_MECH_WEAPON"}')
WHERE definition_kind='EQUIPMENT' AND ruleset_id='${rulesetId}'
  AND definition_id IN ('equipment-mech-heavy-machine-weapon','equipment-mech-autocannon','equipment-mech-light-laser','equipment-mech-medium-laser','equipment-mech-large-laser');
